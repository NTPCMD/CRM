...
| 3 | Next.js app: auth, design system, portals | ✅ Complete — every screen built (`apps/web`): auth + onboarding, role-gated shell, and DB-backed Dashboard, Clients, Pipeline, Projects, Tasks, Calendar, Messages, Files, Contracts, Invoices, Analytics, Team, Permissions editor, Settings; create flows for client/project/invoice/task/lead + live permission matrix; production build passes |
| 4 | API layer, Edge Functions, integrations | 🟡 Next |
